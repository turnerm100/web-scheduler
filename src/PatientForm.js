...
export default function PatientForm({ onClose, onSuccess }) {
  ...
  const [formData, setFormData] = useState({
    ...
    pharmTeam: '', nurseTeam: '', interpreter: '', readWriteLang: '',
    notes: '', lineType: '', ext: '',

    // Blincyto Schedule tab (example placeholders)
    daysInCycle: '', hospStartDate: '', ourStartDate: '', pipsBagChanges: ''
  });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ padding: 20 }}>
      <div style={{ marginBottom: 20 }}>
        <button type="button" onClick={() => setActiveTab('patientInfo')} style={{ marginRight: 10 }}>
          Patient Info
        </button>
        <button type="button" onClick={() => setActiveTab('blincyto')}>
          Blincyto Schedule
        </button>
      </div>
      ...
      {activeTab === 'patientInfo' && (
        <div>
          ...
          <input name="mrn" value={formData.mrn} onChange={handleChange} /><br /><br />
      ...
      {activeTab === 'blincyto' && (
        <div>
          <label>Total Days in Cycle:</label><br />
          <input name="daysInCycle" value={formData.daysInCycle} onChange={handleChange} /><br /><br />

          <label>Hospital Start Date:</label><br />
          <input name="hospStartDate" type="date" value={formData.hospStartDate} onChange={handleChange} /><br /><br />

          <label>Our Start Date:</label><br />
          <input name="ourStartDate" type="date" value={formData.ourStartDate} onChange={handleChange} /><br /><br />

          <label>Are we doing bag changes?</label><br />
